...
#Start using cat + games.csv - Also use IFS
...
do
  if [[ $YEAR != "year" ]]; then
    ...
    if [[ $LOG_GAME == "INSERT 0 1" ]]; then
      echo -e "\nAdded GAME: $YEAR $ROUND: THE WINNER WAS $WINNER ($WINNER_ID) WHICH SCORED $WINNER_GOALS GOALS VERSUS ITS OPPONENT $OPPONENT ($OPPONENT_ID) WHICH SCORED $OPPONENT_GOALS GOALS"
    fi
  fi
done
